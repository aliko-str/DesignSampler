...
var showWarning, showResources, promptComments, useNewSaveMethod, skipWarningsComments;
var forceLazyLoads, lazyLoadsType, retainCrossFrames, mergeCSSImages, executeScripts, removeUnsavedURLs, removeElements, rehideElements, includeInfoBar, includeSummary, formatHTML;
...
var lazyLoadsScrollTime, lazyLoadsShrinkTime;
var maxFrameDepth;
var maxResourceSize;
var maxResourceTime;
var allowPassive;
...
chrome.storage.local.get(null,
	function (object) {
		...
		lazyLoadsType = object["options-lazyloadstype"];
		...
		lazyLoadsScrollTime = object["options-lazyloadsscrolltime"];
		lazyLoadsShrinkTime = object["options-lazyloadsshrinktime"];
...
//		}
		...
		console.log("htmlSaverScripts loaded, listeners are added");
	});
...
function addListeners() {
	...
	// NOTE: all options are assigned in the beginning - we don't have any menues to change them and don't need to
...
//			if ("options-lazyloadstype" in changes) lazyLoadsType = changes["options-lazyloadstype"].newValue;
...
//
//			if ("options-lazyloadsscrolltime" in changes) lazyLoadsScrollTime = changes["options-lazyloadsscrolltime"].newValue;
//			if ("options-lazyloadsshrinktime" in changes) lazyLoadsShrinkTime = changes["options-lazyloadsshrinktime"].newValue;
//
	...
	chrome.runtime.onMessage.addListener(
		function (message, sender, sendResponse) {
			...
			switch (message.type) {
				...
				case "performAction":
					console.log("PERforming action in html saver content script");
					...
					if (document.getElementById("printedit-gui-container") != null) {
						...
						function cancel() {
							if (menuAction <= 1) chrome.runtime.sendMessage({
								type: "saveExit"
							});
						}
		...
		if (pageType < 2) /* not saved page with resource loader */ {
			if (forceLazyLoads) {
				if (lazyLoadsType == 0) /* scroll page */ {
					...
					chrome.runtime.sendMessage({
							type: "delay",
							milliseconds: lazyLoadsScrollTime * 1000
						},
						function timer(object) {
							if (scrolly < document.documentElement.scrollHeight) {
								// console.log("GROWING  -  Inner Height: " + window.innerHeight + "  Scroll Height: " + document.body.scrollHeight + "  Scroll Y: " + scrolly);
								...
								// console.log("FINISHED -  Inner Height: " + window.innerHeight + "  Scroll Height: " + document.body.scrollHeight + "  Scroll Y: " + scrolly);

								window.scrollTo(0, origscrolly);

								endtime = performance.now();

								// console.log("LAZY LOAD TIME: " + (endtime-starttime)/1000 + "secs");

								initializeBeforeSave();
							}
						});
				} else /* lazyLoadsType == 1 */ /* shrink page */ {
					chrome.runtime.sendMessage({
						type: "setSaveState",
						savestate: 0
					});
					...
					document.documentElement.style.setProperty("background", "url(" + lazyLoadData + ") center 20px no-repeat #CCCCCC", "important");
					...
					chrome.runtime.sendMessage({
							type: "delay",
							milliseconds: 500 + lazyLoadsShrinkTime * 1000
						}, /* allow time for page to settle after scroll - e.g. page with floating top bar that resizes when page scrolls */
						function (object) {
							document.documentElement.style.setProperty("background", "#CCCCCC", "important");

							document.body.style.removeProperty("visibility");
							document.body.style.setProperty("transform-origin", "center 10px", "important");
							...
							chrome.runtime.sendMessage({
									type: "delay",
									milliseconds: 500 + lazyLoadsShrinkTime * 1000
								}, /* allow time for first lazy loads to complete */
								function timer(object) {
									if (document.body.scrollHeight > lastscrollheight && document.body.scrollHeight * scale < window.innerHeight) {
										// console.log("GROWING  -  Inner Height: " + window.innerHeight + "  Scroll Height: " + document.body.scrollHeight + "  Scale: " + scale);

										lastscrollheight = document.body.scrollHeight;

										chrome.runtime.sendMessage({
												type: "delay",
												milliseconds: lazyLoadsShrinkTime * 1000
											}, /* allow time for some more lazy loads to complete */
											function (object) {
												timer();
											});
									} else {
										// console.log("FINISHED -  Inner Height: " + window.innerHeight + "  Scroll Height: " + document.body.scrollHeight + "  Scale: " + scale);
...
function initializeBeforeSave() {
	...
	chrome.runtime.sendMessage({
			type: "delay",
			milliseconds: 200
		}, /* allow time for all frames to reply */
		function (object) {
			var i;
...
function rememberCSSURLsInStyleSheet(csstext, baseuri, crossframe, loadedfonts, importstack, framekey) {
	...
	while ((matches = regex.exec(csstext)) != null) /* style sheet imported into style sheet */ {
		if (matches[0].substr(0, 7).toLowerCase() == "@import") /* @import url() */ {
			...
			else fontfamily = removeQuotes(propmatches[1]).toLowerCase();
	...
	for (i = 0; i < resourceLocation.length; i++) {
		if (resourceStatus[i] == "pending") {
			...
			chrome.runtime.sendMessage({
				type: "loadResource",
				index: i,
				location: resourceLocation[i],
				referer: resourceReferer[i],
				passive: resourcePassive[i],
...
function loadShadowLoader() {
	...
	xhr.open("GET", chrome.runtime.getURL("client/htmlSaverScripts/shadowloader-compressed.js"), true);
	xhr.onload = complete;
	xhr.send();
	...
	if (pageType == 0) /* not saved page */ {
		...
		if (dataurisize > maxTotalSize * 1024 * 1024) {
			showMessage("Total size of resources is too large", "Save",
				...
				null,
				function savecancel() {
					if (forceLazyLoads && lazyLoadsType == 1) undoShrinkPage();
...
function enterComments() {
	...
	if (promptComments && !(skipWarningsComments && multipleSaves)) {
		...
		chrome.runtime.sendMessage({
				type: "delay",
				milliseconds: 10
			}, /* allow time for any open panel to close */
			function (object) {
				generateHTML();
			});
	}
	...
	function clickContinue() {
		...
		chrome.runtime.sendMessage({
				type: "delay",
				milliseconds: 10
			}, /* allow time for enter comments panel to close */
			function (object) {
				generateHTML();
			});
	}
...
function generateHTML() {
	...
	if (forceLazyLoads && lazyLoadsType == 1) {
		undoShrinkPage();
	...
	for (i = 0; i < htmlStrings.length; i++) {
		...
		if (htmlString.length >= 1024 * 1024 || i == htmlStrings.length - 1) /* >= 1MB */ {
			prmArr.push(chrome.runtime.sendMessage({
				type: "transferString",
...
//				type: "delay",
//				milliseconds: 100
//			}, /* allow time before revoking object URL */
//			function (object) {
...
function extractHTML(depth, frame, element, crossframe, nosrcframe, framekey, parentpreserve, indent) {
	...
	if (element.localName == "iframe") /* iframe elements */ {
		...
		if (element.localName == "style" || /* <style> element */
			...
			if (shadowroot != null) {
				if (element.localName != "audio" && element.localName != "video" && element.localName != "use") /* ignore standard HTML & SVG elements that have built-in Shadow DOM */ {
					if (pageType == 0 && formatHTML && depth == 0) {
		...
		if (element.localName == "html" || element.localName == "body") {
			...
			if (depth == 0) {
				...
				if (forceLazyLoads && lazyLoadsType == 0) state += " Lazy load scroll steps = " + lazyLoadsScrollTime + "s;";
				if (forceLazyLoads && lazyLoadsType == 1) state += " Lazy load shrink checks = " + lazyLoadsShrinkTime + "s;";
...
function removeResourceLoader() {
	...
	function substituteBlobResources() {
		...
		/* Remove resource loader meta element */
		/* Version 15.0+ */
...
function replaceableResourceURL(url) {
	...
	if (url.substr(0, 5).toLowerCase() == "data:" || url.substr(0, 5).toLowerCase() == "blob:" ||
		url.substr(0, 14).toLowerCase() == "moz-extension:") return false;
...
function getSavedFileName(url, title, extract) {
	var i, documentURL, host, hostw, path, file, query, fragment, datestr, filename, regex, minlength;
	...
	regex = /(%TITLE%|%DATE\((.?)\)%|%TIME\((.?)\)%|%HOST%|%HOSTW%|%PATH%|%FILE%|%QUERY\(([^)]*)\)%|%FRAGMENT%)/g;
